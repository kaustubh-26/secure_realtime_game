...
import drawUI from './ui.mjs';
import generateStartPos from './utils/startPos.mjs';
import drawDropShadow from './utils/dropShadow.mjs';
...
socket.on('connect', () => {
  ...
  const { w, h } = generateStartPos(gameConfig.playFieldOp, gameConfig.avatar);
  opponent = new Player({ w, h, id: socket.id });
...
function renderGame() {
  ...
  // Draw collectible
  if (currentCollectible) {
    currentCollectible.draw(context, collectibleSprites);
  }

  // Draw player's avatar
  if (player) {
    player.draw(context, playerAvatar);
  }

  // Draw each opponent's avatar
  for (const opponent of currentOpponents) {
    opponent.draw(context, opponentAvatar);
  }

  // Draw drop shadow to all sprites on current canvas
  drawDropShadow(context);
